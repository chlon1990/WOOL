    ...
    FileInput.prototype = {
        ...
        readFiles: function (files) {
            ...
            readFile = function (i) {
                ...
                // add file format checking
                if (self.fileType === 0) {
                	let pattern = /^att_ppro_masterfile_[0-9]{8}.xlsx/;
                	if (!pattern.test($h.getFileName(file))) {
                		msg = 'File name should have a format of: att_ppro_masterfile_YYYYMMDD.xlsx, for example: att_ppro_masterfile__20190520.xlsx.';
                		throwError(msg, file, previewId, i, fileId);
                		return;
                	}
                }
                if (self.fileType === 1) {
                	let pattern = /^att_ppro_sitedata_[0-9]{8}.xlsx/;
                	if (!pattern.test($h.getFileName(file))) {
                		msg = 'File name should have a format of: att_ppro_sitedata_YYYYMMDD.xlsx, for example: att_ppro_sitedata_20190520.xlsx.';
                		throwError(msg, file, previewId, i, fileId);
                		return;
                	}
                }
